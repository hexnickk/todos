...
describe('Todos page', () => {
    ...
    //         cy.get('.ant-table-row').each((el) => expect(el).contain.text('ab'));
    //     });
    //
    //     it('should filter by completed', () => {
    //         cy.get('.ant-table-row').should('have.length', 10);
    //         // TODO: move to cy command
    //         cy.getCy('todos-table-filters__completed', '.ant-select-selector').click();
    //         cy.get('.ant-select-item').contains('no').click();
    //         cy.get('.ant-table-row > .ant-table-cell:nth-child(3)').each((el) => expect(el).contain.text('no'));
    //     });
    // });
});
